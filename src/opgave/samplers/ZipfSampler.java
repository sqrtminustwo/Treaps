/*
 * This sampler has been taken from the Apache Commons RNG library and
 * modified to act as a standalone class outside of this library. The
 * original RejectionInversionZipfSampler can be found at this URL:
 * https://gitbox.apache.org/repos/asf?p=commons-rng.git;a=blob;f=commons-rng-sampling/src/main/java/org/apache/commons/rng/sampling/distribution/RejectionInversionZipfSampler.java;hb=HEAD
 *
 ...
 *
 * @See {@code Sampler} for general usage instructions.
 */
public class ZipfSampler extends Sampler {
    ...
    /**
     * Constructor with exponent set to {@code DEFAULT_EXPONENT} (1.0)
     *
     ...
     * or {@code exponent <= 0}.
     */
    public ZipfSampler(Random rng, int numberOfElements) {
        this(rng, numberOfElements, DEFAULT_EXPONENT);
    }
     ...
     * or {@code exponent <= 0}.
     */
    public ZipfSampler(Random rng, int numberOfElements, double exponent) {
        super(rng, numberOfElements);
        if (exponent <= 0) {
    ...
    protected int sample() {
        ...
        while (true) {
            ...
            if (k - x <= s || u >= hIntegral(k + F_1_2) - h(k)) {
                ...
                // Decrement k by one to get a range starting from 0 instead of 1
                return k - 1;
            }
        }
    }
